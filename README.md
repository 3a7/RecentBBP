...
_Sources: bbscope.com (reachable, 24h window by real event timestamps) surfaced 36 in-window "added"/in-scope events after filtering to domain/URL/IP-shaped targets; the rest were Bugcrowd Atlassian-Marketplace-app URLs (Cappsule Sarl, Catapult Labs, codecentric, Creativas, Digital Rose, Railsware, Savignano, Vectors — 34 assets, each a listing URL not a company asset, and unverifiable live since Bugcrowd exposes no public scope API) plus one GitHub repo link — all discarded as out of scope per the asset-type filter/precedent. arkadiyt/bounty-targets-data (24h git-history diff, base 2026-09-10 → head 2026-09-11) independently corroborated `auth.forms-formulaires.alpha.canada.ca` and flagged 4 new Intigriti programs (BMC, Tempo-Team, Yacht, plus a Posti "new program" re-flag) and one new YesWeHack program (Kiteworks, desktop-app assets only, no domains/URLs). Every arkadiyt candidate was diffed against this repo's own master `domains.txt`: 41 of Posti's 43 "new" assets and Kiteworks' 0 domain assets were already-tracked/non-qualifying — Posti's re-flag is the known program-rename churn artifact (identical to prior discarded cases), not new scope, so it was dropped in full. The 8 genuinely new candidates were then checked live: HackerOne's tbs-sct page is a JS-only SPA with no public scope API, so `auth.forms-formulaires.alpha.canada.ca` relies on bbscope's real `2026-09-10T20:01:41Z` add-event timestamp agreeing with arkadiyt's snapshot diff. BMC, Tempo-Team, and Yacht (all Randstad-family Intigriti programs) each show a "launched" activity-log entry dated 9/10 with confidentiality-level updates the same day, and BMC's page explicitly announces "our first bug bounty program" — confirmed as genuine new launches, so `*.bmc.nl`, `www.tempo-team.be`/`.com`/`.nl`, `*.tempo-team.*`, and `*.yacht.nl` were kept (Yacht is VDP/no-bounty). Salto Vulnerability Disclosure's `*.gantner.cloud` is a single incremental addition to an already-established, already-tracked program (64 submissions on record) rather than a full re-add, and was confirmed present in the program's live in-scope wildcard table — kept. Chaos and disclose.io (24h git-history diff) checked, 0 new programs/domains. FireBounty was reachable this run (an initial connection reset was a transient network blip, not a real outage — retried and succeeded) but as a first run in this environment (no snapshot persisted across sessions) only returned the same 10 TUI-group feed ids already discarded in the 2026-09-08/09/10 entries as stale (underlying pages last crawled 2026-06-22, not today) — no new assets attributed to it._
